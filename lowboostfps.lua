-- Low Graphics Toggle GUI (Executor Safe) -- Fokus: GUI pasti muncul di executor, pakai gethui() fallback
...
-- Simpan nilai asli local original = { QualityLevel = nil, GlobalShadows = Lighting.GlobalShadows, Brightness = Lighting.Brightness, FogEnd = Lighting.FogEnd, }

pcall(function() original.QualityLevel = settings().Rendering.QualityLevel end)

local lowGraphicsEnabled = false

-- GUI local screenGui = Instance.new("ScreenGui") screenGui.Name = "LowGraphicsGUI" screenGui.ResetOnSpawn = false

local mainFrame = Instance.new("Frame") mainFrame.Size = UDim2.new(0, 220, 0, 120) mainFrame.Position = UDim2.new(0, 20, 0, 80) mainFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30) mainFrame.BorderSizePixel = 0 mainFrame.Parent = screenGui
...
local fpsLabel = Instance.new("TextLabel") fpsLabel.Size = UDim2.new(1, -20, 0, 20) fpsLabel.Position = UDim2.new(0, 10, 0, 74) fpsLabel.BackgroundTransparency = 1 fpsLabel.Text = "FPS: -- | PING: --" fpsLabel.Font = Enum.Font.SourceSans fpsLabel.TextSize = 14 fpsLabel.TextColor3 = Color3.fromRGB(220,220,220) fpsLabel.TextXAlignment = Enum.TextXAlignment.Left fpsLabel.Parent = mainFrame

-- Dragging local dragging, dragInput, dragStart, startPos
...
-- FPS / Ping update safe mode local fps, frameCount, accumulator = 0, 0, 0 RunService.RenderStepped:Connect(function(dt) frameCount += 1 accumulator += dt if accumulator >= 0.5 then fps = math.floor(frameCount / accumulator + 0.5) frameCount, accumulator = 0, 0

local ping = "--"
    if Stats and Stats:FindFirstChild("Network") then
        local item = Stats.Network.ServerStatsItem:FindFirstChild("Data Ping")
        if item then
            pcall(function() ping = math.floor(item:GetValue() + 0.5) end)
        end
    end

    fpsLabel.Text = string.format("FPS: %s | PING: %s", tostring(fps), tostring(ping))
end

end)

-- Parent ke gethui/CoreGui local suc, res = pcall(function() return gethui() end) screenGui.Parent = (suc and res) or game:GetService("CoreGui")

print("[Low Graphics GUI] Executor Safe Loaded")
