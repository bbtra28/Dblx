...
local function createPlayerButton(p)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if hrp and targetHrp then
            ...
            warn("Teleport gagal: HRP tidak ditemukan.")
        end
    end)
end
...
-- Auto update kalau player join/leave
...
-- Custom drag (hanya bisa drag dari TitleBar)
local dragging, dragStart, startPos
mainFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 
    and input.Position.Y < (titleLabel.AbsolutePosition.Y + titleLabel.AbsoluteSize.Y) then
        dragging = true
